...
const A11Y_RULES = [
  ...
  'Text animations keep the full, readable string as the accessible name (aria-label); the animated fragments are hidden.',
  'Status animations expose the right role — SuccessCheck is role="img" with a label, LoaderDots and ScrollProgress use role="status"/"progressbar".',
...
const REDUCED = [
  ...
  'That switch can’t stop JavaScript motion (requestAnimationFrame loops, timers, pointer tracking), so every animation that runs such logic also checks useReducedMotion() and renders an explicit static fallback.',
...
const REFERENCES = [
  { name: 'React Bits', license: 'MIT', use: 'Inspiration for the animation catalog and individual animation ideas only — no source, assets, branding, or pages were copied.' },
...
export function EffectsGuidePage() {
  usePageTitle('Animation guide');
  return (
    <article>
      <PageHeader
        eyebrow="Animation"
        title="Animation guide"
        description="How FJ uses animation: where it belongs, the performance and accessibility rules every animation follows, and how reduced motion is handled."
      />

      <DocSection id="philosophy" title="Where animation belongs">
      ...
      <DocSection id="inventory" title="What exists">
        <div className="doc-prose">
          ...
          <Stack direction="row" gap={8} wrap>
            ...
            <Badge tone="accent">Status animations</Badge>
            <Badge tone="accent">Animation &amp; transitions</Badge>
          </Stack>
        </div>
      </DocSection>
      ...
      <DocSection id="config" title="Consistent configuration">
        <div className="doc-prose">
          <p>
            Animations share a small, predictable prop vocabulary so they’re easy to learn:{' '}
            ...
            timings all come from FJ tokens, so animations stay on-brand.
          </p>
        </div>
      </DocSection>
